...
def generate_sample_gpu_single_process(
    ...
    if whichset == 'valid' or whichset == 'both':
        ...
        with open(save_dir+'/valid_samples_' + str(uidx) + '.txt', 'w') as f:
            print >>f, '\n'.join(samples_valid)
    if whichset == 'test' or whichset == 'both':
        print 'Test Set...',
        samples_test = sample('test')
        with open(save_dir+'/test_samples_' + str(uidx) + '.txt', 'w') as f:
            print >>f, '\n'.join(samples_test)
...
def compute_score(
        ...
        beam, n_process,
        whichset='both', on_cpu=True, uidx=0,
...
def test_cocoeval():
    ...
    samples_test = common.load_txt_file('result/attr10-log13_2/interact3.txt')
